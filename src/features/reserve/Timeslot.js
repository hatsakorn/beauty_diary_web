...
const selectTime = (time) => {
  ...
  return (<>
    ...
    {timeValue.map(el=>(
        <InputTimeslot key={el.id} timeslot={el.timeslot} onClick={()=>{selectTime(el.timeslot)}} time={time} getTimeslot={getTimeslot} setInputReserve={setInputReserve} inputReserve={inputReserve}/>
        ))}
    </div>
    </>
  )
